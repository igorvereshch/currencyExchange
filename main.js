...
function sort_table() {
    let list_of_children = result.children;
    let recent_child = list_of_children[list_of_children.length - 1];
    let removed_children = [];
    for (let i = list_of_children.length - 2; i > 1; i--) {
        if (parseInt(list_of_children[i].id) > parseInt(recent_child.id)) {
            let child = list_of_children[i];
    ...
    console.log(removed_children);
    for (let j = removed_children.length - 1; j > -1; j--) {
        result.appendChild(removed_children[j]);
    }
}
